...
<div class="space-y-6">
    <div class="flex flex-wrap justify-between items-center gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Tableau de bord citoyen</h1>
            <p class="text-gray-500 text-sm sm:text-base">Suivez vos plaintes et consultez vos notifications</p>
        </div>
        <a href="{{ route('complaints.create') }}" class="bg-blue-700 text-white px-4 sm:px-5 py-3 rounded-xl font-semibold hover:bg-blue-800 text-sm">
            <i class="fa-solid fa-plus mr-2"></i>Nouvelle plainte
        </a>
    </div>

    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl shadow p-4 lg:p-5 border-l-4 border-blue-600">
            <p class="text-gray-500 text-xs sm:text-sm">Total</p>
            <p class="text-2xl sm:text-3xl font-bold text-blue-700">{{ $plaintes->total() ?? 0 }}</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow p-4 lg:p-5 border-l-4 border-blue-500">
            ...
            <p class="text-2xl sm:text-3xl font-bold text-blue-600">{{ $en_cours ?? 0 }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow p-4 lg:p-5 border-l-4 border-green-600">
            <p class="text-gray-500 text-xs sm:text-sm">Résolues</p>
            <p class="text-2xl sm:text-3xl font-bold text-green-600">{{ $resolues ?? 0 }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div class="px-4 lg:px-6 py-4 border-b font-bold text-sm sm:text-base">Mes plaintes</div>
            ...
            <table class="w-full text-xs sm:text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-2 sm:px-4 py-3 text-left hidden sm:table-cell">Service</th>
                        <th class="px-2 sm:px-4 py-3 text-left">Statut</th>
                        <th class="px-2 sm:px-4 py-3 text-left hidden md:table-cell">Date</th>
                ...
                <tbody class="divide-y">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-2 sm:px-4 py-3 font-mono text-blue-700 text-xs">{{ $complaint->code_suivi ?? '#'.$complaint->id }}</td>
                        <td class="px-2 sm:px-4 py-3 font-medium truncate max-w-xs">{{ $complaint->title }}</td>
                        <td class="px-2 sm:px-4 py-3 hidden sm:table-cell">{{ $complaint->service?->name }}</td>
                        <td class="px-2 sm:px-4 py-3">@include('partials.statut-badge', ['statut' => $complaint->statut])</td>
                        <td class="px-2 sm:px-4 py-3 hidden md:table-cell">{{ $complaint->created_at->format('d/m/Y') }}</td>
