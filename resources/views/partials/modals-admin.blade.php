...
<div id="assignModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
        <h3 class="font-bold text-lg mb-4">Affecter à un agent</h3>
        <form method="POST" action="{{ route('admin.assign') }}">
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-blue-700 text-white py-2 rounded-xl">Affecter</button>
...
<div id="statusModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl">
        <h3 class="font-bold text-lg mb-4">Mettre à jour le statut</h3>
        <form method="POST" action="{{ route('admin.update-status') }}">
            ...
            <select name="statut" id="statusSelect" required class="w-full border rounded-xl px-4 py-3 mb-3">
                ...
                <option value="{{ $s }}">{{ PlainteStatut::icon($s) }} {{ $s }}</option>
                @endforeach
            </select>
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-blue-700 text-white py-2 rounded-xl">Enregistrer</button>
